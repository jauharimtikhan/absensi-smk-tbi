...
interface MingguanPageProps extends PageProps {
...
export default function MingguanPage({
    ...
    const fetchData = useCallback(
        async (page?: number) => {
            if (!searchForm.data.tanggal || !searchForm.data.kelas) {
        ...
        async (value: "excel" | "pdf") => {
            ...
            try {
                const response = await window.axios.get(
                    route("laporan.export.mingguan.all"),
                ...
                if (response.status === 200) {
                    ...
                    if (exportType === "excel") {
                        exportStyledExcel({
                            ...
                            tanggal: `${format(
                                searchForm.data.tanggal.start,
                                "dd MMMM",
                                { locale: id }
                            )} - ${format(
                                searchForm.data.tanggal.end,
                                "dd MMMM yyyy",
                                { locale: id }
                            )}`,
                            kelas: kelasLabel,
