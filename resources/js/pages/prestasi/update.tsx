...
import { PrestasiItemType } from ".";

interface PrestasiUpdateModalProps {
    open: boolean;
    onClose: () => void;
    siswas: OptionsSelect2[];
    prestasi: PrestasiItemType;
}
type FormPrestasiUpdate = {
    siswa_id: string;
...
export default function PrestasiUpdateModal({
    open,
    onClose,
    siswas,
    prestasi,
}: PrestasiUpdateModalProps) {
    const { data, setData, errors, processing, put, reset } =
        useForm<FormPrestasiUpdate>({
    ...
    const handleSubmit = (e: FormEvent) => {
        ...
        put(route("prestasi.update", prestasi.id));
    };
